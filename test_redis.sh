...
# Set a test key
redis-cli SET payflow:test:key "test_value" > /dev/null
if [ $? -eq 0 ]; then
...
# Get the test key
VALUE=$(redis-cli GET payflow:test:key)
...
# Test expiration
redis-cli SETEX payflow:test:expire 5 "expires_soon" > /dev/null
if [ $? -eq 0 ]; then
...
# Clean up
redis-cli DEL payflow:test:key payflow:test:expire > /dev/null

echo ""
echo "3. Testing Redis from PayFlow..."
