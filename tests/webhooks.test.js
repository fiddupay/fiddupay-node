...
describe('Webhooks', () => {
    ...
    describe('verifySignature', () => {
        it('should verify valid signature', () => {
            const signature = webhooks_1.Webhooks.generateSignature(payload, secret);
            const isValid = webhooks_1.Webhooks.verifySignature(payload, signature, secret);
            expect(isValid).toBe(true);
        });
        it('should reject invalid signature', () => {
            const invalidSignature = 't=1234567890,v1=invalid';
            const isValid = webhooks_1.Webhooks.verifySignature(payload, invalidSignature, secret);
            expect(isValid).toBe(false);
        });
        it('should reject expired signature', () => {
            const oldTimestamp = Math.floor(Date.now() / 1000) - 400; // 400 seconds ago
            const signature = `t=${oldTimestamp},v1=somehash`;
            const isValid = webhooks_1.Webhooks.verifySignature(payload, signature, secret, 300);
            expect(isValid).toBe(false);
        });
    });
    describe('constructEvent', () => {
        ...
        it('should throw error for invalid signature', () => {
            const invalidSignature = 't=1234567890,v1=invalid';
            expect(() => {
                webhooks_1.Webhooks.constructEvent(payload, invalidSignature, secret);
