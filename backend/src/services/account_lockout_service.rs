...
impl AccountLockoutService {
    ...
    pub async fn record_failed_attempt(&self, email: &str, _ip: &str) -> Result<bool, ServiceError> {
        ...
        let row = sqlx::query("SELECT failed_attempts FROM merchant_login_attempts WHERE email = $1")
            .bind(email)
            .fetch_one(&self.pool)
            .await?;
        ...
        if attempts >= self.max_attempts as i32 {
            let lockout_until = Utc::now() + chrono::Duration::minutes(self.lockout_duration_minutes);
