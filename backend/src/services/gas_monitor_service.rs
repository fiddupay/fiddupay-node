...
impl GasMonitorService {
    ...
    pub async fn start_monitoring(&self) {
        ...
        loop {
            ...
            if let Err(e) = self.check_gas_prices(&http_client, &mut last_alert_times).await {
    ...
    async fn check_gas_prices(
        ...
        let threshold_gwei: Option<Decimal> = settings.try_get("gas_alert_threshold_gwei").ok().flatten();
        let threshold_lamports: Option<i64> = settings.try_get("gas_alert_threshold_lamports").ok().flatten();
        ...
        let evm_networks = vec![CryptoType::Eth, CryptoType::Bnb, CryptoType::Matic, CryptoType::Arb];
            ...
            for crypto in evm_networks {
                if let Ok(gas_wei) = sender.get_current_gas_price(crypto.clone(), false).await {
                    ...
                    if gas_gwei <= target_gwei {
                        let now = Utc::now();
                        let last_alert = last_alert_times.get(&crypto).copied().unwrap_or(now - chrono::Duration::days(1));
        ...
        if let Some(target_lamports) = threshold_lamports {
            if let Ok(lamports) = sender.get_solana_fee(false).await {
                ...
                if (lamports as i64) <= target_lamports {
                    let now = Utc::now();
                    let last_alert = last_alert_times.get(&CryptoType::Sol).copied().unwrap_or(now - chrono::Duration::days(1));
    ...
    async fn send_discord_alert(&self, client: &Client, webhook_url: &str, message: &str) -> Result<(), reqwest::Error> {
        let payload = json!({
            "content": message
        });
