...
impl KeyGenerator {
    ...
    pub fn generate_btc_wallet(is_sandbox: bool) -> Result<WalletKeyPair, ServiceError> {
        ...
        let secret_key = SecretKey::from_slice(&entropy)
            .map_err(|e| ServiceError::Internal(format!("BTC key generation failed: {}", e)))?;
        ...
        let compressed_public_key =
            bitcoin::CompressedPublicKey::try_from(public_key).map_err(|_| {
    ...
    fn validate_solana_private_key(private_key_base58: &str) -> Result<String, ServiceError> {
        ...
        let public_key_bytes = match key_bytes.len() {
            32 => {
                ...
                let keypair = Keypair::from_seed(&seed).map_err(|_| {
                    ServiceError::ValidationError("Invalid Solana seed".to_string())
                })?;
                ...
                let keypair = Keypair::try_from(&bytes[..]) // Use slice reference instead of array
                    .map_err(|_| {
                        ServiceError::ValidationError(
                            "Invalid Solana private key bytes".to_string(),
                        )
                    })?;
                keypair.pubkey().to_bytes()
            }
    ...
    fn validate_btc_private_key(wif: &str) -> Result<String, ServiceError> {
        ...
        let private_key = PrivateKey::from_wif(wif).map_err(|_| {
            ServiceError::ValidationError("Invalid BTC WIF private key".to_string())
        })?;
        ...
        let compressed_public_key =
            bitcoin::CompressedPublicKey::try_from(public_key).map_err(|_| {
                ServiceError::ValidationError(
                    "Invalid BTC public key for compressed format".to_string(),
                )
            })?;
