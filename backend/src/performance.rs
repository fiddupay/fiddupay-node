...
impl BatchOperations {
    /// Batch insert payment_transactions for better performance
    pub async fn batch_insert_payment_transactions(
        pool: &sqlx::PgPool,
        payment_transactions: &[(&str, &str, &str, &str)], // (payment_id, merchant_id, amount_usd, crypto_type)
    ) -> Result<(), sqlx::Error> {
        if payment_transactions.is_empty() {
            return Ok(());
        }

        let mut query_builder = sqlx::QueryBuilder::new(
            "INSERT INTO payment_transactions (payment_id, merchant_id, amount_usd, crypto_type, status, created_at) "
        );
