...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match network.as_str() {
        ...
        _ => println!(
            "Unsupported network: {}. Use BTC, SOL, ETH, BNB, MATIC, ARB.",
            network
        ),
    }
...
async fn simulate_btc(
    ...
    if let Some(inputs) = tx_data["vin"].as_array() {
        for (i, input) in inputs.iter().enumerate() {
            ...
            let addr = prevout["scriptpubkey_address"]
                .as_str()
                .unwrap_or("Unknown");
    ...
    for addr in outputs_list {
        ...
        if let Some(row) = match_address_only {
            ...
            println!(
                "   Amount:     {} req",
                row.get::<Decimal, _>("requested_amount")
            );
            continue;
        }

        let match_merchant_wallet =
            sqlx::query("SELECT merchant_id FROM merchant_wallets WHERE address = $1")
                .bind(&addr)
                .fetch_optional(pool)
                .await?;
        ...
        if let Some(row) = match_merchant_wallet {
            println!(
                "✅ MATCH: Address {} belongs to Merchant {}",
                addr,
                row.get::<i32, _>("merchant_id")
            );
            continue;
        }
    }
...
async fn simulate_solana(
    ...
    if let Some(row) = match_address_only {
        println!(
            "✅ MATCH: Address {} belongs to AddressOnlyPayment!",
            tx.to_address
        );
        println!("   Payment ID: {}", row.get::<String, _>("payment_id"));
        println!("   Status:     {}", row.get::<String, _>("status"));
