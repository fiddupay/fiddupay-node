...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let hash =
        "4jCt18y8JC2UrxkQA6HYPDzKcj7deqNYKBupACSJ2Da8yJNc1vZusT8kYUKogDjmZrHPSMz5NNYAg8zbuwFgYY67";

    println!(
        "🔍 VERIFICATION: Fetching transaction {} for USDT mint {}",
        hash, usdt_mint
    );
    ...
    if tx.amount > Decimal::ZERO {
        ...
        if let Some(mint) = &tx.token_mint {
            if mint == &usdt_mint {
                ...
                println!(
                    "   WARNING: Mint mismatch! Expected {}, got {}",
                    usdt_mint, mint
                );
            }
        }
