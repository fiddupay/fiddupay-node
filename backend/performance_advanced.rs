        ...
        let pool = sqlx::postgres::PgPoolOptions::new()
            ...
            .after_connect(|conn, _meta| {
                Box::pin(async move {
                    ...
                    sqlx::query("SET idle_in_transaction_session_timeout = '60s'").execute(conn).await?;
                    sqlx::query("SET tcp_keepalives_idle = '300'").execute(conn).await?;
                    sqlx::query("SET tcp_keepalives_interval = '30'").execute(conn).await?;
                    sqlx::query("SET tcp_keepalives_count = '3'").execute(conn).await?;
                    Ok(())
                })
            })
...
impl OptimizedQueries {
    ...
    /// Get payments with optimized pagination
    pub async fn get_payments_paginated(
        ...
        sqlx::query_as!(
            ...
            r#"
            SELECT payment_id, merchant_id, amount, amount_usd, crypto_type, 
                   network, status, deposit_address, payment_link, qr_code_data,
                   fee_amount, fee_amount_usd, expires_at, created_at, 
                   confirmed_at, transaction_hash, description
            FROM payments 
            WHERE merchant_id = $1 
