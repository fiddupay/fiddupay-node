...
// Default Solana RPC URL - can be overridden via env variable or system settings
fn get_default_solana_rpc_url() -> String {
...
impl SolanaMonitor {
    pub fn new(rpc_url: Option<String>) -> Self {
        Self {
            client: Client::new(),
            rpc_url: rpc_url.unwrap_or_else(|| get_default_solana_rpc_url()),
        }
    }
    ...
    pub async fn get_transaction_details(
        ...
        Ok(BlockchainTransaction {
            ...
            timestamp: chrono::DateTime::from_timestamp(
                tx_result.block_time.unwrap_or(0),
                0
            ).unwrap(),
            success,
        })
...
mod tests {
    ...
    async fn test_get_current_slot() {
        let monitor = SolanaMonitor::new(None);
